...
def parseInput(input_file):
    ...
    for line in f:
        ...
        photo.append(line[1])
        arrival.append(line[2])
        delay.append(line[3])
    return spot_id, photo, arrival, delay, sensor_count
...
def leaveSpot(s, spot_id):
    ...
    s.connect((HOST_IP, HOST_PORT))
    eprint("Trying to leave spot while connected %s\n" % spot_id) 
    s.send((spot_id + ",StatusChanged,empty").encode())
    #while(1):
    #    data = s.recv(BUFFER_SIZE)
    #    eprint("should be an #A: ", data)
    #    if(data == '#A'):
    #        eprint("Spot %s now empty." % spot_id)
    #        print("Spot %s now empty." % spot_id)
    #        s.close()
    #        break
    data = s.recv(BUFFER_SIZE)
    eprint(spot_id +  "should be an #A: ", data)
    if(data == '#A'):
        eprint("Spot %s now empty." % spot_id)
        print("Spot %s now empty." % spot_id)
        s.close()
...
# arrvial and exit.
def sensorSimulation(spot_id, photo, arrival, delay):
    eprint("arrival= ",arrival  )
    time.sleep(float(arrival))
...
i = 0
spot_id, photo, arrival, delay, sensor_count = parseInput(sys.argv[1])
eprint(sensor_count)
while(sensor_count > i):
    
    spot_id_val, photo_val, arrival_val, delay_val = (spot_id[i], photo[i], arrival[i], delay[i])
    eprint(spot_id_val, photo_val, arrival_val, delay_val)
    #thread.start_new_thread(sensorSimulation (spot_id_val, photo_val, delay_val))
    threading.Thread(target=sensorSimulation, args=(spot_id_val, photo_val, arrival_val,delay_val)).start()
    i = i + 1
