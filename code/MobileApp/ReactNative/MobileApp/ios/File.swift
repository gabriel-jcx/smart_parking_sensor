...
class ViewController : UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate{
  ...
  @objc func BloomEncrypt(_ message:String, callback: @escaping RCTResponseSenderBlock){
    var bloom = Array(repeating: 0, count: 58) //Number of Hash functions is 10 for now
    let input: Array<UInt8> =  Array(message.utf8)
    let key1: Array<UInt8> = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01]
    let key2: Array<UInt8> = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02]
    let key3: Array<UInt8> = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x03]
    let key4: Array<UInt8> = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04]
    let key5: Array<UInt8> = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x05]
    var iv: Array<UInt8> = AES.randomIV(AES.blockSize)
    var big_num:BigInt = 0
    print(iv)
    //print("")
    //var iv_data = NSData(bytes: iv, length: iv.count)
    //iv_data.getBytes(&iv, length: iv.count)
    let iv_string = String(bytes: iv, encoding:String.Encoding.utf8)
    //print(iv_string)
    
    print("Initialization Vector is:", iv)
    iv_array.append(iv)
    do {
      let encrypted = try AES(key: key1, blockMode: .CBC(iv: iv), padding: .pkcs7).encrypt(input)
      //print("The encrypted data is:", encrypted)
      var big_cipher:BigInt = 0
      for i in encrypted{
        print(String(format:"\\x%2x",i), terminator:"")
        if(Int(i) >= 100){
          big_cipher *= 1000
        }else if(Int(i) >= 10){
          big_cipher *= 100
        ...
        //var temp_big:BigInt = Int(i)
        big_cipher += BigInt(Int(i))
      }
      print("")
      print("The big cipher is", big_cipher)
      print(big_cipher%58)
      bloom[Int(big_cipher%58)] += 1
    } catch {
      print(error)
    }
    do {
      let encrypted2 = try AES(key: key2, blockMode: .CBC(iv: iv), padding: .pkcs7).encrypt(input)
      print("The encrypted data is:", encrypted2)
      var big_cipher2:BigInt = 0
      for i in encrypted2{
        if(Int(i) >= 100){
          big_cipher2 *= 1000
        }else if(Int(i) >= 10){
        ...
        big_cipher2 += BigInt(Int(i))
      }
      print("The big cipher 2 is:", big_cipher2)
      print(Int(big_cipher2%58))
      bloom[Int(big_cipher2%58)] += 1
    } catch {
      print(error)
    }
    do {
      let encrypted3 = try AES(key: key3, blockMode: .CBC(iv: iv), padding: .pkcs7).encrypt(input)
      print("The encrypted data is:", encrypted3)
      var big_cipher3:BigInt = 0
      for i in encrypted3{
        if(Int(i) >= 100){
          big_cipher3 *= 1000
        }else if(Int(i) >= 10){
        ...
        big_cipher3 += BigInt(Int(i))
      }
      print("The big cipher 3 is:", big_cipher3)
      print(Int(big_cipher3%58))
      bloom[Int(big_cipher3%58)] += 1
    } catch {
      print(error)
    }
    do {
      let encrypted4 = try AES(key: key4, blockMode: .CBC(iv: iv), padding: .pkcs7).encrypt(input)
      print("The encrypted data is:", encrypted4)
      var big_cipher4:BigInt = 0
      for i in encrypted4{
        if(Int(i) >= 100){
          big_cipher4 *= 1000
        }else if(Int(i) >= 10){
        ...
        big_cipher4 += BigInt(Int(i))
      }
      print("The big cipher 4 is:", big_cipher4)
      print(Int(big_cipher4%58))
      bloom[Int(big_cipher4%58)] += 1
    } catch {
      print(error)
    }
    do {
      let encrypted5 = try AES(key: key5, blockMode: .CBC(iv: iv), padding: .pkcs7).encrypt(input)
      print("The encrypted data is:", encrypted5)
      var big_cipher5:BigInt = 0
      for i in encrypted5{
        if(Int(i) >= 100){
          big_cipher5 *= 1000
        }else if(Int(i) >= 10){
        ...
        big_cipher5 += BigInt(Int(i))
      }
      print("The big cipher 5 is:", big_cipher5)
      print(Int(big_cipher5%58))
      bloom[Int(big_cipher5%58)] += 1
    } catch {
      print(error)
    }
    print("The returning bloom", bloom)
    callback([NSNull(),bloom,iv])
  }
