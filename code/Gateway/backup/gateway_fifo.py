...
def SenorTowerToBuffer(conn, SensorTower, cloud_queue): 
    ...
    spot_id = parsed_data[0] #int(parsed_data[0])
    event = parsed_data[1]
    status = parsed_data[2]
    print ("Spot ID: %s,Task ID: %s, Status: %s" % (spot_id,event,status))
    ...
    if event == 'StatusChanged':
        if(status == 'taken'):
            ...
            conn.send("#P")
            print("Sent Pic request #P")
            print ("Saving test.jpg")
            picFile = open('test.jpg','wb')
        ...
        #time.sleep(0.1)
        #cloud_queue.put(spot_id, status, decoder.get_code("test.jpg"))
        if(gcloud_demo.claim_space(spot_id,decoder.get_code("test.jpg"))):
            conn.send("#A")
            print("#A")
            #conn.close()
            #print("disconnected")
	...
	time.sleep(.500)
        if not cloud_queue.empty: #blocks if it is empty
            ...
            if status == 'taken':
                ...
                print("claim_space(%d,%s) = %d" % (spot_id,qr_code,sensortower[spot_id].cloud_auth_status))
...
#crea new thread for Buffer to Cloud function
